import './Navigation.css';
import { RiMenu3Line, RiCloseLine } from 'react-icons/ri';
import logo from '../../assets/logo.svg'
import { useState } from 'react';
const Navigation = () => {

  const [toggleMenu, setToggleMenu] = useState(false)
  return (
    <div className='gpt3__navigation'>
      <div className='gpt3__navigation-links'>
        <div className='gpt3__navigation-links-logo'>
          <img src={logo} alt='logo' />
        </div>
        <div className='gpt3__navigation-links_container'>
          <p><a href='#home'>Home</a></p>
          <p><a href='#wgpt3'>What is GPT-3</a></p>
          <p><a href='#possibility'>OpenAi</a></p>
          <p><a href='#features'>Casestudies</a></p>
          <p><a href='#blog'>Library</a></p>
        </div>
      </div>
      <div className="gpt3__navigation-sign">
        <p>Sign In</p>
        <button type='submit'>Sign up</button>
      </div>
      <div className='gpt3__navigation-menu'>
        {toggleMenu ? <RiCloseLine color='#fff' size={27} onClick = {()=> setToggleMenu(false)} /> : <RiMenu3Line color='#fff' size={27} onClick = {()=> setToggleMenu(true)} />}
        {toggleMenu && (
        <div className="gpt3__navigation-menu_container scale-up-center">
          <div className="gpt3__navigation-menu_container-links">
            <p><a href="#home">Home</a></p>
            <p><a href="#wgpt3">What is GPT3?</a></p>
            <p><a href="#possibility">Open AI</a></p>
            <p><a href="#features">Case Studies</a></p>
            <p><a href="#blog">Library</a></p>
          </div>
          <div className="gpt3__navigation-menu_container-links-sign">
            <p>Sign in</p>
            <button type="button">Sign up</button>
          </div>
        </div>
        )}
      </div>
    </div>
  );
};

export default Navigation;
